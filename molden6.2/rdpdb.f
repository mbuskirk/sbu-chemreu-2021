...
c D-H..A > 90 degrees
c
      subroutine opthdd(lflp,lhis,lres,lwat,
     &                  iupres,nupres,irsnr,
     ...
     &                        " ",irsnr(ires)
      ...
      if (lflp.eq.1) then
         call flpasn(nupres,iupres,ncalf,icalf,iamino,coo,q,
     &            iconn,ianz,iresid,irsnr,ityp)
      endif
      
      if (lhis.eq.1) then
         call hiseva(nupres,iupres,ncalf,icalf,iamino,coo,q,
     &            iconn,iresid,irsnr,ityp,ipdbt)
      endif

      iff = ifftmp

      if (lwat.eq.1) call evwat
...
c New H coordinates
     ...
     &                  iconn,ianz,iresid,irsnr,ityp)
      ...
      integer*2 ityp
      logical chk2flp,dblflp,flp1,flp2,ck2het,flipem,chkpdb,resok
     ...
     &          dot(3),dnt(3),scori(4),isco(5),isre(4)


      print*,' '
      print*,'ASN and GLN residue flipped if score is 4.0 '
      print*,'points lower than unflipped one'
      print*,' '
      ...
      do i=1,ncalf
         ...
         if (iamino(i).eq.10.or.iamino(i).eq.14) then
            ...
            resok = chkpdb(ipdb,iamino(i),i,irsnr,isl)
            if (resok.and..not.ck2het(ipdb,coo,iconn,ianz,iresid)) then
...
c New H coordinates
                  ...
                  if (.not.dblflp) then
                     ...
                     if (flp1.and.dabs(score2-score1).lt.4.0d0) 
     &                  flp1 = .false.
                  endif
...
c New H coordinates res2
                  ...
                  call rqsrt(4,scori,isco)

                  do k=1,4
                     do j=1,4
                        if (k.eq.isco(j)) isre(k) = j
                     end do
                  end do

                  if (dabs(scori(isco(1))-score1).lt.4.0d0) 
     &                  flipem = .false.
...
c flip res 2
     ...
     &                      '   score'
                  print*,' ------------------------'
                  print*,' unflip  unflip ',score1,' (',isre(1),')'
                  print*,'   flip  unflip ',score2,' (',isre(2),')'
                  print*,'   flip    flip ',score3,' (',isre(3),')'
                  print*,' unflip    flip ',score4,' (',isre(4),')'
                  print*,' ------------------------'
                  print*,' ' 
...
c disconnect HD1 from ND1
      ...
      print*,'The score of HIP is only calculated'
      print*,' '
...
c (mis)use pmf work array to store close residues
     ...
     &          iconn(mxcon+1,*)
      ...
      istart = 1
      do i=1,iatoms
         if (iresid(i).eq.-ishoh.and.ianz(i).eq.8) then
             istart = i
             goto 10
         endif
      end do
10    continue
      ...
      do i=1,iatoms
         if (iresid(i).le.0.and.ianz(i).eq.100) then
             natoms = i-1
             goto 20
         endif
      end do
20    continue
      ...
      i = istart
      if (istart.gt.natoms) natoms = iatoms

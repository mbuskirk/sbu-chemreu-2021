      ...
      if (istatio.eq.1) then
         ...
         if (ioni.ne.1) then
            ...
            if (ivers.lt.2009) then
               ...
               call search(line,'Population analysis using',istat)
               if (istat.eq.1) then
                  call rdvect(idebug,ig94,istat)
                  if (istat.eq.0.and.istato.eq.0) goto 1000
               endif
...
1000  if (idebug.eq.1)  then
      ...
      equivalence (bufft(1),nfast)
      equivalence (bufft(5),nmed)
      equivalence (bufft(9),nslow)
      integer getlin
      character*4 buff(40)
      integer*2 buf2(256)
      character*1 bufft(1024)
      integer*2 idum
...
c         print*,'npts ',npts1,npts2,npts3
      ...
      elseif (ijag.eq.4) then

c ccp4 map

         open(unit=iund,file=keywrd(1:linlen(keywrd)),access="stream")

         read(iund,rec=1) bufft
      ...
      elseif (ijag.eq.5) then

c maestro .plt map

         open(unit=iun,file=keywrd(1:linlen(keywrd)))
...
15       continue
      ...
      nptsmx = mx3d
      if (npts1.gt.nptsmx) nptsmx = npts1
      if (npts2.gt.nptsmx) nptsmx = npts2
      if (npts3.gt.nptsmx) nptsmx = npts3
      if (ijag.eq.3) then
          iptsmx = nptsmx/8
          if (nptsmx.gt.iptsmx*8) nptsmx = (iptsmx+1)*8
      endif

      if (ijag.eq.5) call allgrd2(nptsmx)
...
100   istat = 0
      ...
      subroutine rdccpd(npts1,npts2,npts3,iun,istat,
     &                  denn,dens,pmnn,ichx,a,b,c)
      implicit double precision (a-h,o-z)
     ...
     &                 sl(3),isl
      ...
      integer*4 nfast,nmed,nslow,mode,nsfast,nsmed,nsslow
      integer*4 mx,my,mz,mapc,mapr,maps,ispg,nsymbt
      real*4 abc(3),alpha,beta,gamma,dmin,dmax,dmean
      real*4 ori(3),dtemp,r1,r2,r3

      equivalence (buff(1),nfast)
      equivalence (buff(5),nmed)
      equivalence (buff(9),nslow)
      equivalence (buff(13),mode)
      equivalence (buff(17),nsfast)
      equivalence (buff(21),nsmed)
      equivalence (buff(25),nsslow)
      equivalence (buff(29),mx)
      equivalence (buff(33),my)
      equivalence (buff(37),mz)
      equivalence (buff(41),abc(1))
      equivalence (buff(53),alpha)
      equivalence (buff(57),beta)
      equivalence (buff(61),gamma)
      equivalence (buff(65),mapc)
      equivalence (buff(69),mapr)
      equivalence (buff(73),maps)
      equivalence (buff(77),dmin)
      equivalence (buff(81),dmax)
      equivalence (buff(85),dmean)
      equivalence (buff(89),ispg)
      equivalence (buff(93),nsymbt)
      equivalence (buff(197),ori(1))
      character*1 buff(1024)
      character*1 extbuf(1024)
      dimension n(3),ns(3),dtemp(10000)
      ...
      write(iun3,*) " "
      write(iun3,'(3(a,f7.3))') ,"a=",abc(1)," b=",abc(2)," c= ",abc(3)
      write(iun3,*) " "
      ...
      write(iun3,*) " "
      write(iun3,'(3(a,f7.3))') ,"alpha ",alpha," beta ",beta,
     &                           " gamma ",gamma
      write(iun3,*) " "
      ...
      abc(1) = abc(1) / float(mx)
      abc(2) = abc(2) / float(my)
      abc(3) = abc(3) / float(mz)

      v1(1) = dble(abc(1))
      v1(2) = 0.0d0
      v1(3) = 0.0d0
    
      v2(1) = dble(cos(gamma) * abc(2))
      v2(2) = dble(sin(gamma) * abc(2))
      v2(3) = 0.0d0
    
      z1 = dble(cos(beta))
      z2 = dble((cos(alpha) - cos(beta)*cos(gamma)) / sin(gamma))
      z3 = dble(sqrt(1.0 - z1*z1 - z2*z2));
...
c     nsfast,nsmed,nsslow convert to vo
...
c      print*,"ns(1-3) ",ns(1),ns(2),ns(3)

      vo(1) = v1(1) * dble(ns(mapc)) + 
     &        v2(1) * dble(ns(mapr)) + v3(1) * dble(ns(maps))
      vo(2) = v2(2) * dble(ns(mapr)) + v3(2) * dble(ns(maps))
      vo(3) =                          v3(3) * dble(ns(maps))
...
c     normalise basis vectors
      ...
      r(1) = abc(mapc)*dble(n(mapc))
      r(2) = abc(mapr)*dble(n(mapr))
      r(3) = abc(maps)*dble(n(maps))

      r1 = real(r(mapc))
      r2 = real(r(mapr))
      r3 = real(r(maps))

c nfast,nmed,nslow corresponds to npts1,npts2,npts3
c nsfast,nsmed,nsslow corresponds to 

c      print*," nfast=",nfast," nmed=",nmed," nz=",nslow
c      print*,"mode=",mode
c      print*,"nsfast=",nsfast," nsmed=",nsmed,
c     +         " nsslow=",nsslow
c      print*," mx=",mx," my=",my," mz=",mz
c      print*,"a=",a," b=",b," c=",c
c      print*,"alpha=",alpha," beta=",beta," gamma=",gamma
c      print*,"mapc=",mapc," mapr=",mapr," maps=",maps
      write(iun3,'(3(a,f7.3))') "dmin= ",dmin," dmax= ",dmax,
     &                          " dmean= ",dmean
      write(iun3,*) " "
c      print*,"ispg=",ispg
c      print*,"nsymbt=",nsymbt
c      print*,"ORI=",(ori(i),i=1,3)

c      inquire(iund,pos=ipos)
      read(iund,pos=1024)(extbuf(i),i=1,nsymbt)

c      print*,extbuf(1:nsymbt)

      pmax = -1000000.d0
      pmin = 1000000.d0

c      print*,"npts1,npts2,npts3 ",n(mapc),n(mapr),n(maps)
      npts1 = n(mapc)
      npts2 = n(mapr)
      npts3 = n(maps)
      mxind = mx3d*mx3d2
      ...
      write(iun3,'(a,3(x,i3))') "slow,medium,fast ",
     &                           n(maps),n(mapc),n(mapr)
      write(iun3,*) " "

      do k=1,n(maps)
         ipos = 1024+nsymbt+(k-1)*no*4 + 1
         read(iund,pos=ipos) (dtemp(ii),ii=1,no)

         do i=1,n(mapc)
            do j=1,n(mapr)
                ind = i + (j-1)*n(mapc) + (k-1)*mx3d2

                if (ind.le.mxind) then
                    ii = j + n(mapr)*(i-1)
                    denn(ind) = dtemp(ii)

                    if (dtemp(ii).gt.pmax) pmax = dtemp(ii)
                    if (dtemp(ii).lt.pmin) pmin = dtemp(ii)

                else
                    print*,"ind ",ind," mx3d2 "," error d=",dtemp(ii)
                endif
            end do
         end do
      end do
...
c      print*,"npts ",npts1,npts2,npts3," vlcnt ",vlcnt,vlcnt2
      call mcubes(npts1,npts2,npts3,denn,fdum,mapit,origin,vlcnt,
     &                      vlcnt2,rt,ipsi,dolap,lapdbl,itrans,iun)
...
c     density data
     ...
     &          (couplj((i-1)*natoms+(l-1)*5+j),j=1,nitems)
      ...
      subroutine rdpld(npts1,npts2,npts3,iun,istat,
     &                  denn,dens,pmnn)
      implicit double precision (a-h,o-z)
      common /comsrf/  vo(3), r(3),v1(3),v2(3),v3(3), wo(3),
     &                 sl(3),isl
      ...
      common /curlin/ line
      dimension denn(*),pmnn(*),dens(*),tmp(3),itmp(3)
      ...
      if (linlen(line).ge.5) then
         if (line(1:5).ne."&plot") goto 100
      endif

c do we need to adjust for angstrom or atomic units ?

      do while (getlin(0).eq.1)
         if (linlen(line).eq.44) then

            if (line(1:7).eq."origin=") then
               line = line(8:)
         
               if (gnreal(vo,3,.false.)) then
c                  print*,"vo ",vo(1),vo(2),vo(3)
               endif
            else if (line(1:8).eq."extentx=") then
               line = line(9:)

               if (gnreal(tmp,3,.false.)) then
                  r(1) = tmp(1)
               endif
            else if (line(1:8).eq."extenty=") then
               line = line(9:)
   
               if (gnreal(tmp,3,.false.)) then
                  r(2) = tmp(2)
               endif
            else if (line(1:8).eq."extentz=") then
               line = line(9:)

               if (gnreal(tmp,3,.false.)) then
                  r(3) = tmp(3)
               endif
            else if (line(1:5).eq."npts=") then
               line = line(6:)

               if (gnint(itmp,3,.false.)) then
                  npts1 = itmp(1)
                  npts2 = itmp(2)
                  npts3 = itmp(3)
               endif
...
c get grid value
                  ktype = nxtwrd(str,nstr,itype,rtype)
                  if (ktype.eq.3) then
                     indx = (k-1)*mx3d2 + npts1*(j-1) + i;
                     denn(indx) = rtype
                     if (rtype.gt.pmax) pmax = rtype
                     if (rtype.lt.pmin) pmin = rtype
                  else
                     goto 100
                  endif
               endif
      ...
      vlcnt = 1.00d0
      line = 'found maestro .plt file'

      call inferr(line,0)

      rng1 = pmin
      rng2 = pmax
      write(6,"(2(a,f7.3))") ,"pmin ",pmin," pmax ",pmax

...
This code executes all the functions needed for MyLab1 in a slick, automated
...
psi4.core.set_output_file('MyLab1.dat', False) #this command sets psi4's output to a file. Comment this line out if you want to see the output on the terminal.
...
mol_names = ['Water','Water170','Methanol','MethanolEcl']
...
#example of loading geometry from .xyz file using a for loop
f={}
for mol_name in mol_names:
    f[mol_name] = open(mol_name + '.xyz') #use Python's 'open' to open xyz file.
    molecules[mol_name] = psi4.geometry(f[mol_name].read())
    molecules[mol_name].reset_point_group('c1') # turn symmetry off!
    E0[mol_name] = {} # initialize an empty dictionary to store the E0 results
    Eopt[mol_name] = 0 # initialize optimized energy to scalar 0
...
df.to_csv('MyLab1_Eopt.csv')  # save dataframe to .csv spreadsheet file.

#%%
# Save initial results to a spreadsheet using Pandas
df = pd.DataFrame(E0)
df.to_csv('MyLab1_E0.csv')  # save dataframe to .csv spreadsheet file.
